...
public class TWriter {
    public static String getPreStepContent(TCase tCase, int j) {
        ...
        for(CElement parameter: tCase.getParameters()){
            ...
            index++;
            if(parameter instanceof CVariable && !((CVariable)parameter).values.get(j).isEmpty()){
                sb.append(parameter.getType()).append("*".repeat(((CVariable) parameter).getPointers())).append(" ");
                sb.append(parameterName).append(" = ");
                sb.append(((CVariable) parameter).values.get(j)).append(";\n");
            }else if(parameter instanceof CEnumInstance && !((CEnumInstance)parameter).values.get(j).isEmpty()){
                sb.append(parameter.getType()).append("*".repeat(((CEnumInstance) parameter).getPointers())).append(" ");
                sb.append(parameterName).append(" = ");
                sb.append(((CEnumInstance) parameter).values.get(j)).append(";\n");
                ...
                }else if(!structOrUnionInstance.values.get(j).isEmpty()){
                    sb.append(structOrUnionInstance.getType()).append("*".repeat(structOrUnionInstance.getPointers())).append(" ");
                    sb.append(parameterName).append(" = ");
                    sb.append(structOrUnionInstance.values.get(j)).append(";\n");
                }
            }else if(parameter instanceof CArray array){
                for(CElement element: array.getArrayMembers()){
                    if(element instanceof CVariable && !((CVariable)element).values.get(j).isEmpty()){
                        sb.append(element.getName()).append(" = ");
                        sb.append(((CVariable) element).values.get(j)).append(";\n");
                    }else if(element instanceof CEnumInstance && !((CEnumInstance)element).values.get(j).isEmpty()){
                        sb.append(element.getName()).append(" = ");
                        sb.append(((CEnumInstance) element).values.get(j)).append(";\n");
        ...
        for(CElement inputGlobal: tCase.getInputGlobals()){
            if(inputGlobal instanceof CVariable && !((CVariable)inputGlobal).values.get(j).isEmpty()){
                sb.append(inputGlobal.getName()).append(" = ");
                sb.append(((CVariable) inputGlobal).values.get(j)).append(";\n");
            }else if(inputGlobal instanceof CEnumInstance && !((CEnumInstance)inputGlobal).values.get(j).isEmpty()){
                sb.append(inputGlobal.getName()).append(" = ");
                sb.append(((CEnumInstance) inputGlobal).values.get(j)).append(";\n");
    ...
    private static String _addStructOrUnionInstance(String parameterName,CStructOrUnionInstance structOrUnionInstance, int j){
        ...
        for(CElement element: structOrUnionInstance.getStructType().getMembers()){
            if(element instanceof CVariable && !((CVariable)element).values.get(j).isEmpty()){
                ...
                }else if(!structOrUnionInstance1.values.get(j).isEmpty()){
                    sb.append(parameterName).append(structOrUnionInstance1.getName()).append(" = ");
                    sb.append(structOrUnionInstance1.values.get(j)).append(";\n");
                }
            }else if(element instanceof CArray array){
                for(CElement arrayElement: array.getArrayMembers()){
                    if(arrayElement instanceof CVariable && !((CVariable)arrayElement).values.get(j).isEmpty()){
                        sb.append(parameterName).append(arrayElement.getName()).append(" = ");
                        sb.append(((CVariable) arrayElement).values.get(j)).append(";\n");
                    }else if(arrayElement instanceof CEnumInstance && !((CEnumInstance)arrayElement).values.get(j).isEmpty()){
                        sb.append(parameterName).append(arrayElement.getName()).append(" = ");
                        sb.append(((CEnumInstance) arrayElement).values.get(j)).append(";\n");
    ...
    public static String getPostStepContent(int testCaseIndex,TCase tCase, int j) {
        ...
        if(!tCase.getParent().getTestFunction().getStrType().equals("void")){
            CElement returnType = tCase.getParent().getTestFunction().getRetType();
            if(returnType instanceof CVariable){
                String fprintfCallString = "fprintf(TEST_DATA_FILE, \""+testCaseIndex+"."+j+" "
                                            +returnType.getName()+" %g\\n\", (double)__CTW__RETURN);\n";
                sb.append(fprintfCallString);
            }else if(returnType instanceof CEnumInstance) {
                String fprintfCallString = "fprintf(TEST_DATA_FILE, \"" + testCaseIndex + "." + j + " "
                        + returnType.getName() + " %g\\n\", (double)__CTW__RETURN);\n";
                sb.append(fprintfCallString);
        ...
        for(CElement outputGlobal: tCase.getOutputGlobals()){
            if(outputGlobal instanceof CVariable){
                String fprintfCallString = "fprintf(TEST_DATA_FILE, \""+testCaseIndex+"."+j+" "
                        +outputGlobal.getName()+" %g\\n\", (double)"+outputGlobal.getName()+");\n";
                sb.append(fprintfCallString);
            }else if(outputGlobal instanceof CEnumInstance){
                String fprintfCallString = "fprintf(TEST_DATA_FILE, \""+testCaseIndex+"."+j+" "
                        +outputGlobal.getName()+" %g\\n\", (double)"+outputGlobal.getName()+");\n";
                sb.append(fprintfCallString);
                ...
                for(CElement element: array.getArrayMembers()){
                    if(element instanceof CVariable){
                        String fprintfCallString = "fprintf(TEST_DATA_FILE, \""+testCaseIndex+"."+j+" "
                                +element.getName()+" %g\\n\", (double)"+element.getName()+");\n";
                        sb.append(fprintfCallString);
                    }else if(element instanceof CEnumInstance){
                        String fprintfCallString = "fprintf(TEST_DATA_FILE, \""+testCaseIndex+"."+j+" "
                                +element.getName()+" %g\\n\", (double)"+element.getName()+");\n";
                        sb.append(fprintfCallString);
                    }else if(element instanceof CStructOrUnionInstance structOrUnionInstance){
    ...
    private static String _writeStructOrUnionInstance(String parameterName,CStructOrUnionInstance instance, int testCase,int testStep,String variableName){
        ...
        for(CElement element: instance.getStructType().getMembers()){
            if(element instanceof CVariable){
                ...
                for(CElement arrayElement: array.getArrayMembers()){
                    if(arrayElement instanceof CVariable){
                        String fprintfCallString = "fprintf(TEST_DATA_FILE, \""+testCase+"."+testStep+" "
                                +parameterName+arrayElement.getName()+" %g\\n\", (double)"+variableName+arrayElement.getName()+");\n";
                        sb.append(fprintfCallString);
                    }else if(arrayElement instanceof CEnumInstance){
                        String fprintfCallString = "fprintf(TEST_DATA_FILE, \""+testCase+"."+testStep+" "
                                +parameterName+arrayElement.getName()+" %g\\n\", (double)"+variableName+arrayElement.getName()+");\n";
                        sb.append(fprintfCallString);
